...
func TestDAGInsert(t *testing.T) {
	...
	// i12g has an invalid idle timeout
	...
	// i12h has a reasonable idle timeout
	...
	// i12i has an infinite idle timeout
	...
	tests := map[string]struct {
		...
		"insert service, secret then ingress w/ default tls": {
			...
			want: listeners(
				...
				&Listener{
					Port: 443,
					VirtualHosts: virtualhosts(
						securevirtualhost("kuard.example.com", sec4, prefixroute("/", httpService(s1))),
					),
				},
			),
		},
					...
					VirtualHosts: virtualhosts(
						virtualhost("*", &PrefixRoute{
							...
							Route: Route{
								Clusters: clustermap(s1),
								TimeoutPolicy: &TimeoutPolicy{
									IdleTimeout: -1, // invalid max_grpc timeout equals infinity ¯\_(ツ)_/¯.
								},
							},
						}),
					),
		...
		"insert ingress w/ valid idle annotation": {
			objs: []interface{}{
				i12h,
				s1,
			},
		...
		"insert ingress w/ infinite idle annotation": {
			objs: []interface{}{
				i12i,
				s1,
			},
