...
func TestRouteVisit(t *testing.T) {
	tests := map[string]struct {
			...
			objs: []interface{}{
				&v1beta1.Ingress{
					ObjectMeta: metav1.ObjectMeta{
						...
						Namespace: "default",
						Annotations: map[string]string{
							"contour.heptio.com/idle-timeout": "heptio",
						},
					},
					Spec: v1beta1.IngressSpec{
						Backend: &v1beta1.IngressBackend{
			...
			want: map[string]*v2.RouteConfiguration{
				"ingress_http": {
					...
					VirtualHosts: []route.VirtualHost{{
						...
						Routes: []route.Route{{
							Match:               envoy.PrefixMatch("/"),
							Action:              routeidletimeout("default/kuard/8080/da39a3ee5e", duration(0)),
							RequestHeadersToAdd: envoy.RouteHeaders(),
						}},
					}},
				},
		...
		"ingress 90 second idle timeout": {
			objs: []interface{}{
				&v1beta1.Ingress{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "kuard",
						Namespace: "default",
						Annotations: map[string]string{
							"contour.heptio.com/idle-timeout": "1m30s",
						},
					},
					Spec: v1beta1.IngressSpec{
						Backend: &v1beta1.IngressBackend{
			...
			want: map[string]*v2.RouteConfiguration{
				"ingress_http": {
					...
					VirtualHosts: []route.VirtualHost{{
						...
						Routes: []route.Route{{
							Match:               envoy.PrefixMatch("/"),
							Action:              routeidletimeout("default/kuard/8080/da39a3ee5e", duration(90*time.Second)),
							RequestHeadersToAdd: envoy.RouteHeaders(),
						}},
					}},
				},
...
func routeidletimeout(cluster string, idleTimeout *time.Duration) *route.Route_Route {
	r := routecluster(cluster)
	r.Route.IdleTimeout = idleTimeout
	return r
}
