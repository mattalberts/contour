	...
	tests := map[string]struct {
		...
		"--envoy-http-access-log": {
			...
			want: listenermap(&v2.Listener{
				...
				FilterChains: filterchain(envoy.HTTPConnectionManager(ENVOY_HTTP_LISTENER, "/tmp/http_access.log", envoy.HTTPConnectionOptions{})),
			}, &v2.Listener{
				Name:    ENVOY_HTTPS_LISTENER,
				...
				FilterChains: []listener.FilterChain{{
					...
					TlsContext: tlscontext(auth.TlsParameters_TLSv1_1, "h2", "http/1.1"),
					Filters:    filters(envoy.HTTPConnectionManager(ENVOY_HTTPS_LISTENER, "/tmp/https_access.log", envoy.HTTPConnectionOptions{})),
				}},
			}),
		},
	}
