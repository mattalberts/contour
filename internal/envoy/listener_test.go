...
func TestHTTPConnectionManager(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			got := HTTPConnectionManager(tc.routename, tc.accesslog, HTTPConnectionOptions{})
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Fatal(diff)
			}
		})
...
func TestTCPProxy(t *testing.T) {
			...
			want: listener.Filter{
				...
				ConfigType: &listener.Filter_TypedConfig{
					TypedConfig: any(&envoy_config_v2_tcpproxy.TcpProxy{
						...
						IdleTimeout: tvd(0, TCPDefaultIdleTimeout),
					}),
				},
			},
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			got := TCPProxy(statPrefix, tc.proxy, accessLogPath, TCPProxyOptions{})
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Fatal(diff)
			}
		})
