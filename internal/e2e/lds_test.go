	...
	assertEqual(t, &v2.DiscoveryResponse{
		...
		Resources: []types.Any{
			any(t, &v2.Listener{
				...
				FilterChains: filterchain(envoy.HTTPConnectionManager("ingress_http", "/dev/stdout", envoy.HTTPConnectionOptions{})),
			}),
			any(t, staticListener()),
		},
		TypeUrl: listenerType,
...
func TestLDSCustomAccessLogPaths(t *testing.T) {
	...
	ingress_http := &v2.Listener{
		Name:         "ingress_http",
		Address:      *envoy.SocketAddress("0.0.0.0", 8080),
		FilterChains: filterchain(envoy.HTTPConnectionManager("ingress_http", "/tmp/http_access.log", envoy.HTTPConnectionOptions{})),
	}
	ingress_https := &v2.Listener{
		Name:    "ingress_https",
		Address: *envoy.SocketAddress("0.0.0.0", 8443),
