	...
	assertEqual(t, &v2.DiscoveryResponse{
		...
		Resources: []types.Any{
			...
			any(t, &v2.Listener{
				...
				FilterChains: filterchaintls("kuard.example.com", envoy.HTTPConnectionManager("ingress_https", "/dev/stdout", 0, false), "h2", "http/1.1"),
			}),
		},
		TypeUrl: listenerType,
		Nonce:   "0",
...
func TestLDSCustomAccessLogPaths(t *testing.T) {
	...
	ingress_https := &v2.Listener{
		...
		FilterChains: filterchaintls("kuard.example.com", envoy.HTTPConnectionManager("ingress_https", "/tmp/https_access.log", 0, false), "h2", "http/1.1"),
	}
	assertEqual(t, &v2.DiscoveryResponse{
		VersionInfo: "0",
		Resources: []types.Any{
...
func TestIngressRouteHTTPS(t *testing.T) {
	...
	ingressHTTPS := &v2.Listener{
		...
		FilterChains: filterchaintls("example.com", envoy.HTTPConnectionManager("ingress_https", "/dev/stdout", 0, false), "h2", "http/1.1"),
	}
	assertEqual(t, &v2.DiscoveryResponse{
		VersionInfo: "0",
		Resources: []types.Any{
