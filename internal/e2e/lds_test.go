	...
	assertEqual(t, &v2.DiscoveryResponse{
		...
		Resources: resources(t,
			&v2.Listener{
				...
				FilterChains: envoy.FilterChains(envoy.HTTPConnectionManager("ingress_http", "/dev/stdout", envoy.HTTPConnectionOptions{})),
			},
			staticListener(),
		),
		TypeUrl: listenerType,
	...
	assertEqual(t, &v2.DiscoveryResponse{
		...
		Resources: []types.Any{
			...
			any(t, &v2.Listener{
				...
				FilterChains: filterchaintls("kuard.example.com", s1, envoy.HTTPConnectionManager("ingress_https", "/dev/stdout", envoy.HTTPConnectionOptions{}), "h2", "http/1.1"),
			}),
			any(t, staticListener()),
		},
		TypeUrl: listenerType,
...
func TestIngressRouteTLSListener(t *testing.T) {
	...
	l1 := &v2.Listener{
		...
		FilterChains: filterchaintls("kuard.example.com", secret1, envoy.HTTPConnectionManager("ingress_https", "/dev/stdout", envoy.HTTPConnectionOptions{}), "h2", "http/1.1"),
	}
...
func TestLDSCustomAccessLogPaths(t *testing.T) {
	...
	ingress_http := &v2.Listener{
		Name:         "ingress_http",
		Address:      *envoy.SocketAddress("0.0.0.0", 8080),
		FilterChains: envoy.FilterChains(envoy.HTTPConnectionManager("ingress_http", "/tmp/http_access.log", envoy.HTTPConnectionOptions{})),
	}
	ingress_https := &v2.Listener{
		Name:    "ingress_https",
		Address: *envoy.SocketAddress("0.0.0.0", 8443),
		...
		FilterChains: filterchaintls("kuard.example.com", s1, envoy.HTTPConnectionManager("ingress_https", "/tmp/https_access.log", envoy.HTTPConnectionOptions{}), "h2", "http/1.1"),
	}
	assertEqual(t, &v2.DiscoveryResponse{
		VersionInfo: "1",
		Resources: []types.Any{
...
func TestIngressRouteHTTPS(t *testing.T) {
	...
	ingressHTTPS := &v2.Listener{
		...
		FilterChains: filterchaintls("example.com", s1, envoy.HTTPConnectionManager("ingress_https", "/dev/stdout", envoy.HTTPConnectionOptions{}), "h2", "http/1.1"),
	}
	assertEqual(t, &v2.DiscoveryResponse{
		VersionInfo: "1",
		Resources: []types.Any{
